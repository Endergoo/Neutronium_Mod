...
{
    public class GammaRayBurstBeam : ModProjectile
    {
        // A helpful math constant for performing beam angling calculations.
        private const float PiBeamDivisor = MathHelper.Pi / GammaRayBurstHoldout.NumBeams;
        ...
        public override void AI()
        {
            ...
            if (Projectile.type != ModContent.ProjectileType<GammaRayBurstBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<GammaRayBurstHoldout>())
            {
                Projectile.Kill();
                return;
            }
            ...
            float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / GammaRayBurstHoldout.MaxCharge, 0f, 1f);
            ...
            Projectile.friendly = hostPrism.ai[0] > GammaRayBurstHoldout.DamageStart;

            // This offset is used to make each individual beam orient differently based on its Beam ID.
            float beamIdOffset = BeamID - GammaRayBurstHoldout.NumBeams / 2f + 0.5f;
            float beamSpread;
            float spinRate;
            ...
            float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * GammaRayBurstHoldout.NumBeams) * MathHelper.TwoPi;
        ...
        private Color GetOuterBeamColor()
        {
            ...
            float hue = (BeamID / GammaRayBurstHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
