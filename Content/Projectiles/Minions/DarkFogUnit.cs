...
namespace Neutronium.Content.Projectiles.Minions
{
    public class DarkFogUnit : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                owner.ClearBuff(ModContent.BuffType<DarkFogUnitBuff>());
                return;
            }

            if (owner.HasBuff(ModContent.BuffType<DarkFogUnitBuff>()))
            {
        ...
        {
            // Make the sprite face the opposite direction of its movement
            if (Projectile.velocity.LengthSquared() > 0.1f) // Avoid rotation jitter when velocity is very small
            {
                Projectile.rotation = Projectile.velocity.ToRotation() + MathHelper.Pi; // Flip the sprite
            }

            int frameSpeed = 5; // Speed of the animation frame changes
            ...
            // Add a light effect around the projectile
